...
C4GroupSet &C4Language::GetPackGroups(const char *strRelativePath)
{
	...
	if (Game.GameC4S.Head.Origin.getLength() && SEqualNoCase(GetExtension(Game.GameC4S.Head.Origin.getData()), "c4s"))
	{
		...
		if (szScenarioRelativePath != strRelativePath)
		{
			...
			if (Game.GameC4S.Head.Origin.getLength() + 1 + iRestPathLen <= _MAX_PATH)
			{
				SCopy(Game.GameC4S.Head.Origin.getData(), strTargetLocation);
				if (iRestPathLen)
				{
