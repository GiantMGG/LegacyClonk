...
#include <C4GuiButton.h>
#include <C4GuiResource.h>
#include <C4Log.h>

C4ReplayViewerDlg::C4ReplayViewerDlg()
	: C4GUI::Dialog(nullptr, LoadResStr("Replay Viewer"),
	                0, 0, 0, C4ReplayViewerDlg::kOverlayHeight)
{
...
void C4ReplayViewerDlg::OnPlayPause()
{
...
void C4ReplayViewerDlg::OnStepForward()
{
	GetController().StepForward();
}

void C4ReplayViewerDlg::OnStepBackward()
{
...
void C4ReplayViewerDlg::OnClose()
{
	Close(false);
}

void C4ReplayViewerDlg::Draw(C4Facet &cgo, float iDrawZ)
{
	C4GUI::Dialog::Draw(cgo, iDrawZ);
	DrawTimelineBar(cgo);
}
...
void C4ReplayViewerDlg::DrawTimelineBar(C4Facet &cgo)
{
	...
	// Background
	Application.DDraw->DrawBox(cgo.Surface, barX, barY, barX + barW, barY + barH, 0xff404040);
	// Filled portion
	const int32_t fillW = static_cast<int32_t>(barW * fFill);
	Application.DDraw->DrawBox(cgo.Surface, barX, barY, barX + fillW, barY + barH, 0xff00ff00);

	// Tick marks at every 10% of total frames.
	for (int32_t pct = 10; pct < 100; pct += 10)
	{
		const int32_t tickX = barX + (barW * pct) / 100;
		Application.DDraw->DrawBox(cgo.Surface, tickX, barY, tickX + 1, barY + barH, 0xff808080);
	}

	// Playhead
	const int32_t headX = barX + fillW;
	Application.DDraw->DrawBox(cgo.Surface, headX - 1, barY - 2, headX + 2, barY + barH + 2, 0xffffffff);
}
