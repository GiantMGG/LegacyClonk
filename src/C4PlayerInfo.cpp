...
bool C4PlayerInfo::LoadFromLocalFile(const char *szFilename)
{
	...
	assert(!Game.GameC4S.Head.Replay);
	// clear previous
	Clear();
	...
	if (Game.Network.isEnabled() && !Game.GameC4S.Head.Replay)
	{
		// add ressource
...
void C4PlayerInfo::LoadResource()
{
	...
	if (Game.GameC4S.Head.Replay || (dwFlags & PIF_InScenarioFile))
		dwFlags &= ~PIF_HasRes;
	else
		...
		if (!(pRes = Game.Network.ResList.AddByCore(ResCore)))
		{
			...
			LogNTr(spdlog::level::err, "Could not add resource {} for player {}! Player file too large to load?", ResCore.getID(), GetFilename());
		}
}
	...
	while ((pPlrInfo = pNewClientInfo->GetPlayerInfo(i++)))
		if (!pPlrInfo->GetID())
		{
			...
			if (GetFreePlayerSlotCount() - iJoinsGranted < 1)
			{
				...
				Log(C4ResStrTableKey::IDS_MSG_TOOMANYPLAYERS, static_cast<int>(Game.Parameters.MaxPlayers));
	...
	if ((szPos = SSearch(pSource, "[PlayerFiles]")))
		while (true)
		{
			...
			if (SEqual2(szLinebuf, "Player") && (SCharPos('=', szLinebuf) > -1))
			{
				...
				{
					Log(C4ResStrTableKey::IDS_ERR_LOAD_PLRINFO, szPlayerFilename);
					delete pNewInfo;
				}
				else
				{
...
{
	// not in replay
	if (Game.GameC4S.Head.Replay) return;
	// no double init
	assert(!GetInfoCount());
	// no network
	...
	while ((pInfo = pkLocal->GetPlayerInfo(i++)))
		...
		if (!pInfo->HasJoinIssued())
		{
			...
			if (!szFilename && (pInfo->GetType() != C4PT_Script))
			{
				...
				Log(C4ResStrTableKey::IDS_ERR_JOINQUEUEPLRS, szPlrName);
				continue;
			}
	...
	for (i = 0; i < rSavegamePlayers.GetInfoCount(); ++i)
	{
		...
		while ((pInfo = pkInfo->GetPlayerInfo(j++)))
			if (pInfo->GetType() == C4PT_Script)
			{
				...
				if (!pHostInfo)
				{
					LogNTr(spdlog::level::err, "Error restoring savegame script players: No host player infos to add to!");
					continue;
				}
	...
	if (rSavegamePlayers.GetPlayerCount())
	{
						...
						if (!(id = pInfo->GetAssociatedSavegamePlayerID()))
							if ((pSavegameInfo = rSavegamePlayers.FindSavegameResumePlayerInfo(pInfo, static_cast<MatchingLevel>(eMatchingLevel), static_cast<MatchingLevel>(eMatchingLevel))))
							{
								...
								if (eMatchingLevel > PML_PlrName)
								{
									...
									LogNTr(msg);
									if (Game.pGUI && FullScreen.Active && !Game.GameC4S.Head.Replay)
		...
		for (i = 0; i < iClientCount; ++i)
		{
			...
			while ((pInfo = pkInfo->GetPlayerInfo(j++)))
				...
				else
				{
					...
					if (Game.GameC4S.Head.SaveGame) Log(C4ResStrTableKey::IDS_PRC_RESUMENOPLRASSOCIATION, pInfo->GetName());
				}
		}
		...
		if (iCountRemaining)
		{
			...
			else
			{
				...
				Log(C4ResStrTableKey::IDS_PRC_RESUMEREMOVEPLRS, iCountRemaining);
	...
	for (int32_t i = 0; i < iClientCount; ++i)
	{
		...
		while ((pInfo = pkInfo->GetPlayerInfo(j++)))
			if (pInfo->IsJoined())
			{
				...
				if (Game.GameC4S.Head.Replay || Game.GameC4S.Head.NetworkRuntimeJoin || pInfo->GetType() == C4PT_Script)
				{
					...
					if (!szCurrPlrFile || !*szCurrPlrFile)
					{
						...
						if (pInfo->GetType() != C4PT_Script)
						{
							Log(C4ResStrTableKey::IDS_ERR_LOAD_RECR_NOFILE, szPlrName);
						}
						continue;
					}
					// join from temp file
					...
					if (!Game.ScenarioFile.FindEntry(GetFilename(szCurrPlrFile)) || !Game.ScenarioFile.Extract(GetFilename(szCurrPlrFile), szJoinPath.getData()))
					{
						...
						if (pInfo->GetType() != C4PT_Script)
						{
							Log(C4ResStrTableKey::IDS_ERR_LOAD_RECR_NOEXTRACT, szPlrName, GetFilename(szCurrPlrFile));
						}
						continue;
					}
					// set join source
		...
		if (Game.GameC4S.Head.Replay)
			...
			else
			{
				...
				else
				{
					...
					Log(C4ResStrTableKey::IDS_PRC_RESUMENOCLIENT, idAtClient, pkInfo->GetPlayerCount());
					continue;
				}
			}
			...
			if (pInfo->IsJoined())
			{
				...
				if (!szFilename || !*szFilename)
				{
					if (pInfo->GetType() == C4PT_User)
					{
						...
						const char *szPlrName = pInfo->GetName(); if (!szPlrName) szPlrName = "???";
						Log(C4ResStrTableKey::IDS_ERR_LOAD_RECR_NOFILEFROMNET, szPlrName);
						continue;
					}
					else
					{
	...
	while ((pClient = rSavegamePlayers.GetIndexedInfo(iClient++)))
	{
		...
		while ((pInfo = pClient->GetPlayerInfo(iInfo++)))
		{
			...
			if (pInfo->IsJoined() && !GetPlayerInfoBySavegameID(pInfo->GetID()))
			{
				...
				{
					Log(C4ResStrTableKey::IDS_PRC_REMOVEPLR, pInfo->GetName());
				}
				else
					fSuccess = false;
			}
